...
export function useConfig() {
  ...
  async function initSystemConfigs() {
    ...
    await configStore.fetchPermissions()
  }

  /**
   * 验证配置值
   */
  async function validateConfigValue(configKey: string, configValue: string): Promise<ConfigValidation> {
    validating.value = true
    try {
      const response = await configApi.systemConfig.validateConfig(configKey, configValue)
   ...
   * 批量保存系统配置
   */
  async function batchSaveSystemConfigs(configs: Array<{ configKey: string; configValue: string; description?: string }>) {
    // 批量验证
    ...
    saving.value = true
    try {
      await configStore.batchUpdateSystemConfigs(configs)
      return true
    } catch (error) {
      return false
    } finally {
   ...
   * 测试配置连接
   */
  async function testConfigConnection(configKey: string) {
    loading.value = true
    try {
      const response = await configApi.systemConfig.testConfig(configKey)
      if (response.data.success) {
        ElMessage.success(`${configKey} 连接测试成功`)
      } else {
        ElMessage.error(`${configKey} 连接测试失败: ${response.data.message}`)
      }
    ...
    try {
      ...
      if (response.data.success) {
        if (response.data.restartRequired) {
          ElMessage.warning('配置已应用，但需要重启服务才能完全生效')
        } else {
  ...
  async function getSystemHealth() {
    try {
      const response = await configApi.getHealthStatus()
      return response.data
    } catch (error) {
