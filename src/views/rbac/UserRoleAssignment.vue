...
const loadUserRoles = async (userId: number) => {
  try {
    const res = await rbacApi.getUserRoles(userId) as any
...
const handleAssignRole = async (user: AdminUser) => {
  ...
  try {
    const res = await rbacApi.getUserRoles(user.id) as any
...
const handleSubmitRoles = async () => {
  ...
  try {
    await rbacApi.assignRolesToUser(currentUser.value.id, {
