...
const handleEdit = async (row: AdminUser) => {
  ...
  try {
    const rolesRes = await rbacApi.getUserRoles(row.id)
    // API 返回格式: { code, message, data: Role[] }
    ...
    try {
      if (isEdit.value) {
        ...
        if (formData.role_ids && formData.role_ids.length > 0) {
          await rbacApi.assignRolesToUser(formData.id, {
        ...
        if (formData.role_ids && formData.role_ids.length > 0 && res.id) {
          await rbacApi.assignRolesToUser(res.id, {
            role_ids: formData.role_ids,
            is_primary: true
          })
        }
