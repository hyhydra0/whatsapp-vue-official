...
async function loadAccounts() {
  try {
    const res = await whatsappApi.getAccounts()
    if (res && Array.isArray(res)) {
      connectedAccounts.value = res.filter((a: any) => a.connected)
    }
  } catch (error: any) {
...
async function loadChats() {
  ...
  try {
    const res = await whatsappApi.getAccountChats(store.selectedAccountId)
    if (res && Array.isArray(res)) {
      chats.value = res
