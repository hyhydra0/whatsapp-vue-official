...
export const systemConfigApi = {
  // 获取所有系统配置（按分组）
  getConfigs(): Promise<ApiResponse<SystemConfigGroup[]>> {
    return request.get('/config/system')
  },

  // 获取特定分组的配置
  getConfigsByCategory(category: string): Promise<ApiResponse<SystemConfig[]>> {
    return request.get(`/config/system/category/${category}`)
  },

  // 搜索配置项
  searchConfigs(params: ConfigSearchParams): Promise<ApiResponse<PaginatedResponse<SystemConfig>>> {
    return request.get('/config/system/search', params)
  },

  // 获取单个配置项
  getConfig(configKey: string): Promise<ApiResponse<SystemConfig>> {
    return request.get(`/config/system/${configKey}`)
  },

  // 更新单个配置项
  updateConfig(configKey: string, data: ConfigUpdateRequest): Promise<ApiResponse<SystemConfig>> {
    return request.put(`/config/system/${configKey}`, data)
  },

  // 批量更新配置项
  batchUpdateConfigs(data: ConfigBatchUpdateRequest): Promise<ApiResponse<SystemConfig[]>> {
    return request.put('/config/system/batch', data)
  },

  // 验证配置值
  validateConfig(configKey: string, configValue: string): Promise<ApiResponse<ConfigValidation>> {
    return request.post('/config/system/validate', { configKey, configValue })
  },

  // 重置配置为默认值
  resetConfig(configKey: string): Promise<ApiResponse<SystemConfig>> {
    return request.post(`/config/system/${configKey}/reset`)
  },

  // 测试配置连接（如数据库、邮件等）
  testConfig(configKey: string): Promise<ApiResponse<{ success: boolean; message: string }>> {
    return request.post(`/config/system/${configKey}/test`)
  }
}
