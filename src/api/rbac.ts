...
export const rbacApi = {
  ...
  getUserRoles(userId: number): Promise<ApiResponse<Role[]>> {
    return request.get<Role[]>(`/rbac/users/${userId}/roles`)
  },
  ...
  getUserPermissions(userId: number): Promise<ApiResponse<Permission[]>> {
    return request.get<Permission[]>(`/rbac/users/${userId}/permissions`)
  },

  /**
   * 为用户分配角色
   */
  assignRolesToUser(userId: number, data: AssignRolesToUserRequest): Promise<ApiResponse> {
  ...
  checkUserPermission(
    ...
    return request.get<CheckPermissionResponse>(`/rbac/users/${userId}/check-permission`, {
      params
    })
  },
